...
export const ModalUserEdit = (props) => {

    const theme = useTheme();
    const colors = tokens(theme.palette.mode);
    const { fetchData, handleCloseModalEditUser, OpenModalEditUser, DataUserToAction } = props;
    ...
    const { id, name, user, profile, status, url_image } = DataUserToAction;

    const { control, reset, handleSubmit, getValues, formState: { errors } } = useForm();

    useEffect(() => {
        reset(DataUserToAction);

    }, [DataUserToAction]);

    const dispatch = useDispatch();

    const { EditUsers } = useUserStore()
    ...
    const handleSubmitEvent = (data, event) => {
        ...
        const response = EditUsers(DataRequest);
    ...
    return (
        <Modal open={OpenModalEditUser} onClose={handleCloseModal}>
            <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 400, bgcolor: "background.paper", boxShadow: 24, p: 4, backgroundColor: theme.palette.mode === "dark" ? colors.primary[500] : colors.grey[900] }}>
                ...
                <form onSubmit={handleSubmit(handleSubmitEvent)}>
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <CssFormControlPassword sx={{ my: 1 }} fullWidth error={Boolean(errors.status)}>
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <CssFormControlPassword sx={{ my: 1 }} fullWidth error={Boolean(errors.profile)}>
                                <InputLabel id="select-label">-Seleccione perfil-</InputLabel>
                                <Select
                                    labelId="select-label"
                                    id="profile"
                                    name="profile"
